...
4. Add Environment Variables:
   ...
   - `QWEN_API_KEY` - DashScope API key (same value as `DASHSCOPE_API_KEY`)
   - `QWEN_MODEL` - `qwen-plus`
   - `QWEN_BASE_URL` - `https://dashscope-intl.aliyuncs.com/compatible-mode/v1`
     - China (Beijing) key: `https://dashscope.aliyuncs.com/compatible-mode/v1`
   - `GEMINI_API_KEY` - Legacy Gemini key (not used for chat once Qwen is set)
