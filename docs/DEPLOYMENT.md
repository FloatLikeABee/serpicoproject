...
4. Add Environment Variables:
   ...
   - `SILICONFLOW_API_KEY` - SiliconFlow API key (or `QWEN_API_KEY`)
   - `QWEN_MODEL` / `SILICONFLOW_MODEL` - `deepseek-ai/DeepSeek-V4-Flash`
   - `QWEN_BASE_URL` / `SILICONFLOW_BASE_URL` - `https://api.siliconflow.com/v1` for keys from siliconflow.com. Keys created on siliconflow.cn only work on `https://api.siliconflow.cn/v1`. The backend retries the other host on 401.
   - `GEMINI_API_KEY` - Legacy Gemini key (not used for chat)
