...
func scanFleetMarker(scanner interface {
	...
	err := scanner.Scan(
		...
		&row.Lat, &row.Lng, &address, &notes, &row.CreatedAt, &row.UpdatedAt,
	)
	if err != nil {
		return row, err
	}
...
func loadFleetMarker(db *database.Database, id, userID string) (fleetMarkerRow, error) {
	row := db.SQLite.QueryRow(`
		SELECT id, user_id, city_id, kind, name, lat, lng, address, notes, created_at, updated_at
		FROM fleet_markers
...
func handleFleetCreateMarker(c *gin.Context, db *database.Database) {
	...
	_, err := db.SQLite.Exec(`
		INSERT INTO fleet_markers (id, user_id, city_id, kind, name, lat, lng, address, notes, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		id, userID, cityID, kind, name, *req.Lat, *req.Lng, address, notes, now, now)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	c.JSON(http.StatusCreated, gin.H{"marker": row})
}
...
func handleFleetUpdateMarker(c *gin.Context, db *database.Database) {
	...
	_, err = db.SQLite.Exec(`
		...
		SET city_id = ?, kind = ?, name = ?, lat = ?, lng = ?, address = ?, notes = ?, updated_at = ?
		WHERE id = ? AND user_id = ?`,
		cityID, kind, name, lat, lng, address, notes, now, id, userID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
