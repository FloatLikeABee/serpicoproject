...
test('officer Navigation and Login have no hardware-registry or handle link; /x-hard-data remains', () => {
  ...
  expect(nav).not.toMatch(/\/x-hard-data\/hw/);
  expect(login).not.toMatch(/\/hardware/);
  expect(login).not.toMatch(/\/x-hard-data\/hw/);
  expect(app).toMatch(/path="\/x-hard-data"/);
  expect(app).toMatch(/path="\/x-hard-data\/hw\/:serial"/);
});
